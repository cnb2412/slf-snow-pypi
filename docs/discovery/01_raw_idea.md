...
typed access to current snow data from Switzerland's automated measurement
stations.
...
- Retrieve **snow depth (HS)** and **new snow over the last 24 h (HN24)** for
  selected stations.
...
- The library can list stations and fetch HS and HN24 for selected stations.
...
- Potentially additional measurement values (e.g. temperature, wind) beyond
  snow data.
